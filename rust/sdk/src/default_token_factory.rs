...
impl DefaultTokenFactory {
    ...
    /// the subject JWT's remaining lifetime. If the subject is not a decodable
    /// JWT with a numeric `exp`, or `expires_in` is absent, the original
    /// `expires_in` is returned unchanged — the server already validated the
    /// exchange, so falling back never serves a worse token than before. A
    /// decodable-but-already-expired subject caps the TTL to zero, so the cache
    /// treats the token as dead on arrival (a retryable re-mint) rather than
    /// serving it past the subject's expiry.
    ...
    /// Remaining lifetime from a JWT's **unverified** `exp` claim: `Some(exp -
    /// now)` for a future `exp`, `Some(Duration::ZERO)` when `exp` is already
    /// past (or clock-skewed), and `None` only when the token is not a readable
    /// JWT with a numeric `exp`. The signature is deliberately NOT verified:
    /// `exp` is used only to shorten our own cache TTL, never to authorize — the
    /// server validates the subject token during the exchange. `exp` is an RFC
    /// 7519 NumericDate (seconds since the Unix epoch).
    fn jwt_exp_remaining(token: &str) -> Option<Duration> {
        // A JWT is `header.payload.signature`; the payload (second segment) is
        // base64url-encoded JSON. The spec uses unpadded base64url, but some
        // issuers emit padded (`=`) payloads, so accept both rather than silently
        // failing to decode (which would drop the cap and keep UC's `expires_in`).
        ...
        // `exp` is an RFC 7519 NumericDate: usually an integer, but the spec allows
        // a non-integer (float) value, which `as_u64` would miss. Accept an integer
        // directly, else a finite float truncated to whole seconds (sub-second
        // precision is irrelevant for a token TTL). A finite non-positive value is a
        // readable already-expired subject, so clamp it to 0 (which yields a ZERO
        // remaining lifetime below) rather than dropping the cap and keeping UC's
        // `expires_in`.
        let exp_claim = claims.get("exp")?;
        let exp_secs = exp_claim.as_u64().or_else(|| {
            let secs = exp_claim.as_f64()?;
            if !secs.is_finite() {
                return None;
            }
            Some(if secs <= 0.0 { 0 } else { secs as u64 })
        })?;
        let exp = Duration::from_secs(exp_secs);
        let now = SystemTime::now().duration_since(UNIX_EPOCH).ok()?;
        // A readable-but-already-expired subject yields `ZERO`, not `None`, so the
        // TTL cap becomes zero and the cache treats the exchanged token as dead on
        // arrival (a retryable re-mint) rather than caching a token past its
        // subject's expiry. `None` is reserved for a genuinely undecodable subject,
        // where we can only fall back to `expires_in`.
        exp.checked_sub(now).or(Some(Duration::ZERO))
    }
...
mod tests {
    ...
    /// Like [`jwt_with_exp`] but base64url-encodes the payload WITH padding
    /// (`=`), which the spec permits and some issuers emit. The extra claim makes
    /// the payload length not a multiple of 3, so the encoding actually carries
    /// padding (asserted, so a future content change can't silently drop it).
    fn jwt_with_padded_exp(exp_secs: u64) -> String {
        let payload = URL_SAFE.encode(format!(r#"{{"exp":{},"iss":"z"}}"#, exp_secs));
        assert!(
            payload.ends_with('='),
            "test helper must produce a padded payload, got {payload}"
        );
        format!("eyJhbGciOiJub25lIn0.{}.sig", payload)
    }

    /// Like [`jwt_with_exp`] but `exp` is a non-integer NumericDate (a JSON
    /// float), which RFC 7519 permits.
    fn jwt_with_float_exp(exp_secs: f64) -> String {
        let payload = URL_SAFE_NO_PAD.encode(format!(r#"{{"exp":{}}}"#, exp_secs));
        format!("eyJhbGciOiJub25lIn0.{}.sig", payload)
    }
    ...
    #[test]
    fn ttl_capped_to_zero_when_subject_already_expired() {
        // A decodable subject whose `exp` is already past caps the TTL to zero, so
        // the cache treats the exchanged token as dead on arrival and re-mints
        // rather than caching a token past its subject's expiry for up to an hour.
        let subject = jwt_with_exp(now_secs() - 60);
        let capped =
            DefaultTokenFactory::cap_ttl_to_subject_jwt(Some(Duration::from_secs(3600)), &subject);
        assert_eq!(capped, Some(Duration::ZERO));
    }

    #[test]
    fn ttl_capped_when_subject_jwt_payload_is_padded() {
        // A padded base64url payload must still decode, so the cap applies. Before
        // the padded-fallback fix this decode failed and the TTL silently kept the
        // UC-reported 3600s.
        let subject = jwt_with_padded_exp(now_secs() + 120);
        let capped =
            DefaultTokenFactory::cap_ttl_to_subject_jwt(Some(Duration::from_secs(3600)), &subject)
                .unwrap();
        assert!(
            capped <= Duration::from_secs(120) && capped >= Duration::from_secs(110),
            "expected ~120s from a padded payload, got {:?}",
            capped
        );
    }

    #[test]
    fn ttl_capped_when_subject_jwt_exp_is_a_float() {
        // A non-integer NumericDate (float `exp`) must still be read, so the cap
        // applies. Before accepting floats, `as_u64` returned None and the TTL
        // silently kept the UC-reported 3600s.
        let subject = jwt_with_float_exp((now_secs() + 120) as f64 + 0.5);
        let capped =
            DefaultTokenFactory::cap_ttl_to_subject_jwt(Some(Duration::from_secs(3600)), &subject)
                .unwrap();
        assert!(
            capped <= Duration::from_secs(121) && capped >= Duration::from_secs(110),
            "expected ~120s from a float exp, got {:?}",
            capped
        );
    }

    #[test]
    fn ttl_capped_to_zero_when_subject_jwt_exp_is_a_negative_float() {
        // A finite non-positive NumericDate is a readable already-expired subject,
        // so it must clamp to ZERO (dead on arrival, re-mint) like an integer past
        // `exp`. Before clamping, `as_u64` missed the negative value and the
        // `>= 0.0` guard returned None, silently dropping the cap and keeping 3600s.
        let subject = jwt_with_float_exp(-1.0);
        let capped =
            DefaultTokenFactory::cap_ttl_to_subject_jwt(Some(Duration::from_secs(3600)), &subject);
        assert_eq!(capped, Some(Duration::ZERO));
    }
