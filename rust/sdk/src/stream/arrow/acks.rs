...
impl AckProgress<'_> {
    ...
    async fn apply(&self, ack: &FlightAckMetadata) -> ZerobusResult<()> {
        ...
        // Dispatch before publishing the acknowledged offset so wait_for_offset/flush
        // cannot succeed ahead of the export callback. Channel delivery is best-effort;
        // this does not wait for a telemetry consumer to process the event.
        if let Some(exporter) = self.stats_exporter {
            for offset in acked_offsets {
                crate::stats::dispatch(exporter.as_ref(), StreamStat::BatchAcked { offset });
            }
        }
