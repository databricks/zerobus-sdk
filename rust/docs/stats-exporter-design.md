...
```rust
#[non_exhaustive]
pub struct BatchStats { pub records: u64, pub approximate_wire_bytes: u64, pub uncompressed_bytes: u64 }

#[non_exhaustive]
#[derive(Clone)]
pub enum StreamStat {
    BatchSent    { offset: OffsetId, attempt: u32, stats: BatchStats }, // 0 = first send, >0 = retransmit
    BatchAcked   { offset: OffsetId },                                  // durability only
    Reconnected  { reason: ReconnectReason },                           // TransientFailure(err) | ServerRotation
}
...
so new stats land in minor releases. `StreamStat` is owned and `Clone` (not `Copy`,
so a future variant can carry a non-`Copy` payload such as an owned error), so it
forwards across a channel with no borrowed/owned split.
...
- `uncompressed_bytes` sums the emitted IPC buffer lengths before compression,
  including dictionary buffers when sent. IPC metadata supplies buffer lengths;
  compressed buffers carry their original length in an eight-byte prefix (`-1`
  means the following bytes are stored raw). This follows the encoder's handling
  of all array layouts and slices, with no payload copying or re-encoding. It
  excludes IPC metadata, alignment padding, and compression length prefixes.
  `approximate_wire_bytes` is summed from the emitted
  `FlightData` payloads (after IPC compression) — Arrow payload only, so it is a lower
  bound on network bytes (excludes gRPC/HTTP2/TLS framing and the schema message).
- These live in a **single current-batch accumulator**. IPC record-batch headers
  supply the row counts for each split frame; dictionary messages contribute bytes
  without advancing the row count. `BatchSent` is emitted with the frame that
  completes the batch's row count, without waiting for another poll, input batch,
  or EOF. Later cancellation or drop cannot discard that completed event.
  Transmissions canceled before their final frame do not emit `BatchSent`.
- Consequences: `BatchSent` **counts completed retransmissions**.
  A replay reuses the original `offset` and carries the whole batch if no rows were
  acknowledged, otherwise only the still-unacked suffix. Its byte sizes include all
  frames emitted by that completed retry. Event totals exclude bytes from earlier
  incomplete attempts. If the first transmission was incomplete, the first event
  can have `attempt > 0`, so filtering for `attempt == 0` can omit a batch entirely.
  An original-batch size sample is not guaranteed for every offset.
  Durability is a separate signal (`BatchAcked`). The transmission counter is shared
  by pending and outbound copies of a batch and advances only on the first emitted
  data frame. Preparing or queuing a replay does not advance it, so a batch buffered
  during recovery still has `attempt == 0` on its first send.
...
- `StreamStat` is owned and `Clone`, so `record` forwards it across the channel
