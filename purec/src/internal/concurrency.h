...
typedef struct zb_mutex zb_mutex_t;
typedef struct zb_cond zb_cond_t;
typedef struct zb_thread zb_thread_t;
 ...
 * zb_mutex_free and zb_cond_free accept NULL; success frees the handle,
 * while failure leaves it allocated.
 */
...
/* If already locked, returns OK with *out_locked == false. */
 ...
 * may start before zb_thread_new returns. Data returned by the worker must
 ...
 * Success waits for completion, makes the worker's writes visible, and frees
 * the handle; failure leaves the handle and out_result unchanged.
 * On success, a non-NULL out_result receives the worker's returned pointer.
 * Pass NULL for out_result to ignore that pointer.
 */
