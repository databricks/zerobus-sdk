...
def get_entra_token():
    ...
    proactive refresh), not on every record.

    Caveat: a synchronous callback like this runs inline and holds the interpreter
    (GIL) while it blocks, so this network call stalls the SDK for its whole
    duration and is not interrupted by `recovery_timeout_ms`. (The same is true on
    the async SDK: a sync callback runs before the await point, so it blocks the
    event loop just the same.) That is acceptable for an occasional mint like this,
    but for heavier or less reliable IdP I/O use the async SDK with an `async def`
    callback (which the SDK awaits without holding the GIL), or return an
    already-cached token here.
    """
    resp = requests.post(
